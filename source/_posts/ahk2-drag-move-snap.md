...
这个脚本经历了 6 轮修 bug，以下是核心教训：

### 坑 1：OnMessage 吞掉所有点击

最初用 `OnMessage(0x0201, ...)` 拦截 `WM_LBUTTONDOWN` 来判断拖拽——结果全局左键事件全被拦截，`return 0` 阻止消息继续传递，窗口完全点不动。

**解法**：用 `LButton & RButton` 组合热键 + `~LButton` 前缀保留左键正常功能，完全避开消息拦截。

### 坑 2：AHK v2 的 Object 字面量语法

```autohotkey
; ❌ 某些上下文中报错 "Missing propertyname:"
obj := {x: 1, y: 2}

; ✅ Map() 参数中可以用，其他地方必须加引号
obj := {"x": 1, "y": 2}
```

`{key: value}` 写法只在 `Map()`、`OnMessage()` 等特定上下文生效。v1 → v2 最常见绊脚石。

### 坑 3：Return 导致死代码

AHK v2 的静态分析器比 v1 严格，循环中提前 `return` 后写的代码会被标记为 unreachable。

### 坑 4：多版本迭代互相覆盖

一个脚本改了 5 版以上，旧的中断逻辑和新的热键处理互相打架。解决方式：每次大改直接重写，不在已有代码上叠补丁。

核心原则：**涉及鼠标拦截的场景，优先用组合热键而不是消息钩子。** 热键是 AHK 最成熟的抽象，OnMessage 是全局钩子——出错的代价是整台机器的鼠标事件。
